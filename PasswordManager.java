...
class SplashScreen {
    ...
    public void createGUI(){
        ...
        frame.getContentPane().setBackground(new Color(0XFF8787)); // to set the background color of the frame 0XFF8787 is light pink color
    ...
    public void addText()
    {
        ...
        text.setForeground(Color.black);//Password Manager color
        frame.add(text);
    }
    ...
    public void addProgressBar(){
        ...
        progressBar.setBackground(Color.black);//loading background color
        progressBar.setForeground(new Color(0X38E54D));//green color
...
class HashtablePassword implements hashTableMap {
    ...
    public int hash(Object key){
        return (key.hashCode() & 0x7FFFFFFF) % entries.length;//electric blue color
    }
        ...
        if(PassGeneBtn ==e.getSource())
        {
            try{
                ...
                if(len>4)
                {
                    ...
                    JOptionPane.showMessageDialog(conn1,new JScrollPane(genePassArea),"",JOptionPane.INFORMATION_MESSAGE);//Copy your password

                }
                else JOptionPane.showMessageDialog (conn1,"Password length must be greater than 5","Invalid Input Error",JOptionPane.WARNING_MESSAGE);
